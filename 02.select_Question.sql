...
*/   
select deptno, ename, sal*12 from emp e where deptno = 10;


/**2.
사원번호가 7369인 사람 중 이름, 입사일, 부서 번호를 출력하라.
*/
select ename, hiredate, deptno from emp e where empno = 7369;

/**3.
이름이 ALLEN인 사람의 모든 정보를 출력하라.
*/
select * from emp e where ename = 'ALLEN';


/**4.
입사일이 83/01//12인 사원의 이름, 부서번호, 월급을 출력하라.
*/
select ename, deptno, sal from emp e where '1983-01-12';


/**5.
직업이 MANAGER가 아닌 사람의 모든 정보를 출력하라.
*/
select * from emp e where job not in ('MANAGER');

/**6.
입사일이 81/04/02 이후에 입사한 사원의 정보를 출력하라.
*/
select * from emp e where hiredate > '1981-04-02';
...
*/
select ename, sal, deptno from emp e where sal >= 900;
...
*/
select * from emp e where hiredate < '1981-12-09';
...
*/
select empno, ename from emp e where empno <= 7968;


/**12.
입사일이 81/04/02보다 늦고  82/12/09보다 빠른 사원의 이름, 월급, 부서번호를 출력하라.
*/
select ename, sal, deptno from emp e where hiredate between '1981-04-02' and '1982-12-09';
...
*/
select ename, job, sal from emp e where sal >1600 and sal <3000;
...
*/
select * from emp e where empno not between 7654 and 7782;
...
*/
select ename, empno, deptno from emp e where deptno not in (20,30);
...
*/
select empno, ename, hiredate, deptno from emp e where ename like 'S%';
...
*/
select * from emp e where ename like 'S___T%';
...
*/
select ename, deptno, sal from emp e where deptno = 30 and sal > 1500;
...
*/
select empno, ename, deptno from emp e where deptno = 30 or ename like 'K%';
...
*/
select * from emp e where sal >= 1500 and deptno = 30 and job = 'MANAGER';


/**24.
부서번호가 30인 사람중 사원번호 정렬하라.
*/
select empno from emp e where deptno = 30 order by empno desc;
select empno from emp e where deptno = 30 order by empno asc;
...
*/
select * from emp e order by deptno asc, sal asc;
